...
export class StatsService {
  ...
  async getDashboardStats() {
    ...
    // Busca nomes dos produtos populares em uma única consulta
    ...
    const popularProductsWithNames = popularProducts.map((p) => ({
      name: productMap.get(p.productId) || 'Desconhecido',
      quantity: p._sum.quantity
    }));

    // Vendas nos últimos 7 dias para o gráfico - uma única consulta
